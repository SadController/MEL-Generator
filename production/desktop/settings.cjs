...
const defaults = {schemaVersion:SCHEMA_VERSION, aircraft:'A321', count:2, width:1180,
  height:860, maximized:false, checkForUpdatesOnStartup:true,
  activateFailuresOnBriefing:false, enableDiagnosticLog:false};
function sanitize(value) {
  ...
  return {
    ...
    checkForUpdatesOnStartup:typeof v.checkForUpdatesOnStartup === 'boolean'
      ? v.checkForUpdatesOnStartup : defaults.checkForUpdatesOnStartup,
    activateFailuresOnBriefing:v.activateFailuresOnBriefing === true,
    enableDiagnosticLog:v.enableDiagnosticLog === true
  };
}
