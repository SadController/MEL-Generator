...
const CATALOG=Object.freeze({
  ...
  SOURCE_MISSING:{title:'The MMEL document is missing',message:'The source document required for this card is not installed.',action:'Reinstall MEL Generator from the official release.',severity:'error',retryable:false},
  SOURCE_OPEN_FAILED:{title:'The MMEL document could not be opened',message:'The installed source document is present but its viewer could not be started.',action:'Close the source window and try again. Reopen MEL Generator if the problem continues.',severity:'error',retryable:true},
  ...
  AIRCRAFT_UNSUPPORTED:{title:'Automatic activation is unavailable for this aircraft',message:'The loaded aircraft does not have a supported failure adapter.',action:'Load a supported Fenix A319, A320 or A321, or activate the briefing manually.',severity:'warning',retryable:true},
  FENIX_UNAVAILABLE:{title:'Fenix failure manager is unavailable',message:'MEL Generator could not reach the local Fenix failure interface.',action:'Confirm that a Fenix aircraft is fully loaded, then retry. You can still activate the briefing manually.',severity:'warning',retryable:true},
  FENIX_MAPPING_INCOMPLETE:{title:'A generated failure is not mapped',message:'The installed Fenix failure catalogue does not contain a required record.',action:'Do not retry automatically. Activate the marked failure manually and update MEL Generator.',severity:'error',retryable:false},
  ACTIVATION_REJECTED:{title:'Fenix rejected automatic activation',message:'At least one failure is armed or active in a state that MEL Generator cannot safely replace.',action:'Review the Fenix failure manager, clear conflicting entries and retry, or activate the briefing manually.',severity:'warning',retryable:true},
  ACTIVATION_NOT_CONFIRMED:{title:'Failure activation was not confirmed',message:'The command was sent, but the Fenix failure manager did not confirm the requested state.',action:'Review the per-card status and activate unconfirmed failures manually before flight.',severity:'error',retryable:true},
  ACTIVATION_ROLLBACK_FAILED:{title:'Failure rollback was incomplete',message:'Automatic activation failed and at least one newly changed failure could not be restored.',action:'Open the Fenix failure manager and review every failure in this briefing before continuing.',severity:'error',retryable:false},
...
function classifyError(scope,error) {
  ...
  if(scope==='activation') {
    ...
    if(/ECONNREFUSED|127\.0\.0\.1|8083|gateway|timed? ?out|connection/i.test(text)) return issue('FENIX_UNAVAILABLE');
    if(/mapping|mapped|catalogue is missing|missing:|failure record/i.test(text)) return issue('FENIX_MAPPING_INCOMPLETE');
